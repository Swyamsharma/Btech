...
vector<int> merge(vector<int> &arr1, vector<int> &arr2, vector<int> &arr3, vector<int> &arr4)
{
    ...
    while (i < arr1.size() || j < arr2.size() || k < arr3.size() || l < arr4.size())
    {
        ...
        if (k < arr3.size())
            mini = min(mini, arr3[k]);
        if (l < arr4.size())
            mini = min(mini, arr4[l]);

        ans.push_back(mini);

        if (i < arr1.size() && arr1[i] == mini)
            i++;
        else if (j < arr2.size() && arr2[j] == mini)
            j++;
        else if (k < arr3.size() && arr3[k] == mini)
            k++;
        else
            l++;
    }
...
int main()
{
    vector<int> arr = {100, 2000, 425, 1, 25, 98, 69, 699, 6969, 69, 420, 421};
